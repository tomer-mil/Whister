...
| App switch mid-trick | Trick totals must catch up while an observer is away. | Background observer while three other pages claim tricks. | `e2e/tests/mobile/lifecycle.spec.ts::S1: app-switch away during trick claiming; trick counts update on return` | ❌ gap | S1 fails in this run: `playing-trick-count-1` (and seats 2, 3) timed out after 15 s on the returning observer's page; trick count DOM elements were not found. See [MR-12](#mr-12-app-switch-mid-trick-does-not-update-trick-counts-on-return). |
...
| REST timeout/error handling | A failed score fetch must not silently show stale/empty data. | Abort score-table route during round completion. | `e2e/tests/mobile/network.spec.ts::N3: score-table fetch blocked mid-request; UI shows error (not silent failure)` | ✅ verified | N3 passes: aborting the score-table route now surfaces a visible error state (not silent failure). Full-suite run: 63 passed / 65 total. |
| Full round on slow 3G | Latency and low bandwidth must not break game progression. | CDP 250 kbps down/50 kbps up/300 ms RTT. | `e2e/tests/mobile/network.spec.ts::N4: full round completes under 3G throttle (250kbps, 300ms RTT)` | ✅ verified | N4 passes: full round completes under 3G throttle (250 kbps down / 50 kbps up / 300 ms RTT) in 46 s. Full-suite run: 63 passed / 65 total. |
...
| Browser kill / relaunch state | OS process eviction must restore auth, room, phase, and turn. | Close context, create a fresh context from live storage state, navigate to room. | `e2e/tests/mobile/pwa.spec.ts::P1: JWT auth token persists across context close + reopen`; `e2e/tests/mobile/recovery.spec.ts::R3: context close + new context; JWT from storage state re-authenticates automatically` | ⚠️ partial | R3 passes (JWT re-authenticates and state restores). P1 fails: after context reopen, `bidding-current-turn` / `bidding-pass` did not become visible within 20 s; bidding phase was not restored. See [MR-13](#mr-13-p1-bidding-phase-not-restored-after-context-reopen). |
...
| PWA manifest/install metadata | Standalone launch needs valid metadata and resolvable icons. | Fetch manifest and every declared icon. | `e2e/tests/mobile/pwa.spec.ts::P2: /manifest.json is served with required PWA fields` | ✅ verified | P2 passes: manifest is served with all required PWA fields and declared icons resolve with 2xx responses. Full-suite run: 63 passed / 65 total. |
| Standalone/offline relaunch | An installed game should recover after browser eviction or radio loss. | Inspect service-worker registrations/controller. | `e2e/tests/mobile/pwa.spec.ts::P3: installed scope has an active service worker for offline relaunch` | ✅ verified | P3 passes: an active service worker registration and controller are now present in the installed scope. Full-suite run: 63 passed / 65 total. |
| Session/token persistence | Relaunch must not force login in the middle of a game. | Live `storageState` transferred into a new mobile context. | `e2e/tests/mobile/recovery.spec.ts::R3: context close + new context; JWT from storage state re-authenticates automatically`; `e2e/tests/mobile/pwa.spec.ts::P1: JWT auth token persists across context close + reopen` | ⚠️ partial | R3 passes: JWT re-authenticates and state restores via new context. P1 fails in this run: bidding phase not visible after context reopen within 20 s. See [MR-13](#mr-13-p1-bidding-phase-not-restored-after-context-reopen). |
...
| Connection indicator accuracy | Players need to know whether an action can reach the server. | Compare recovery UI with actual socket behavior after reload/navigation. | `e2e/tests/mobile/touch.spec.ts::G1: browser back then forward recovers the active mobile game`; `e2e/tests/mobile/pwa.spec.ts::Th2: socket reconnects after a page reload under 3G throttle` | ✅ verified | G1 passes: back/forward navigation no longer leaves the indicator Disconnected; bid controls are reachable on return. Th2 also passes. Full-suite run: 63 passed / 65 total. |

## Known gaps / app-side defects

### MR-12 App-switch mid-trick does not update trick counts on return

`S1` backgrounds the observer (P0) while the other three players claim tricks, then foregrounds P0.
The test times out waiting for `playing-trick-count-1` (and seats 2, 3) to appear — those data-testid
elements were not found within 15 s on P0's page. The trick-count DOM is either not rendered in the
playing phase after a synthetic background/foreground or the socket events driving it are not replayed
on reconnect. Full-suite run (63/65): S1 fails.

### MR-13 P1: bidding phase not restored after context close + reopen

`P1` closes the browser context (simulating a kill/relaunch) and reopens it from saved storage state.
The JWT re-authenticates (the token-present assertion passes), but `bidding-current-turn` /
`bidding-pass` do not become visible within 20 s. The socket reconnects but `sync:state` either does
not fire or the Zustand hydration does not set the bidding-phase DOM in time. R3 (a parallel test
covering the same scenario via a fresh context) passes, so the failure is specific to the P1 flow.
Full-suite run (63/65): P1 fails.
