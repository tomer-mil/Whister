...
Rounds represent individual dealing/playing cycles within a game,
including trump bidding, contract bidding, and trick tracking.
"""
...
class Round(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    optional frisch phases, contract bidding, and play phases.

    Attributes:
        id: UUID primary key
        game_id: The game this round belongs to
        round_number: Round number within the game (1-indexed)
        phase: Current phase of the round
        trump_suit: The winning trump suit (null during bidding)
        trump_winner_id: User who won the trump bid
        trump_bid_amount: The winning trump bid amount
        frisch_count: Number of frisch rounds (0-3)
        minimum_bid: Current minimum bid (increases after frisch)
        game_type: Over/under based on contract sum
        total_contracts: Sum of all contract bids
        current_bidder_seat: Seat position of current bidder
        consecutive_passes: Number of consecutive passes in trump bidding
        total_tricks_played: Total tricks completed (0-13)
    ...
    __table_args__ = (
        ...
        # Bidder seat must be 0-3 when set
        CheckConstraint(
            "current_bidder_seat IS NULL OR "
            "(current_bidder_seat >= 0 AND current_bidder_seat <= 3)",
            name="current_bidder_seat_valid",
        ),
...
class RoundPlayer(Base, UUIDPrimaryKeyMixin):
    ...
    Attributes:
        id: UUID primary key
        round_id: The round this record belongs to
        user_id: The user this record is for
        seat_position: Seat position (0-3, denormalized)
        contract_bid: Player's contract bid (0-13, null if not yet bid)
        bid_order: Order in which player bid (1-4)
        tricks_won: Number of tricks won this round
        score: Score for this round (calculated)
        made_contract: Whether player made their contract
    """
    ...
    __table_args__ = (
        ...
        # Bid order must be 1-4 when set
        CheckConstraint(
            "bid_order IS NULL OR (bid_order >= 1 AND bid_order <= 4)",
            name="bid_order_valid",
        ),
...
class TrumpBid(Base, UUIDPrimaryKeyMixin):
    ...
    Attributes:
        id: UUID primary key
        round_id: The round this bid was made in
        player_id: The user who made the bid
        amount: Bid amount (0 for pass, 5-13 for actual bids)
        suit: Bid suit (null for pass)
        is_pass: Whether this was a pass
        created_at: When the bid was made
    """
    ...
    __table_args__ = (
        # Bid must be valid: either a pass (amount=0, no suit) or actual bid (amount>=5, has suit)
        CheckConstraint(
