...
Groups represent recurring player collections (e.g., "Friday Night Whist")
for aggregated statistics and easy game setup.
"""
...
class Group(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    players. A group can have up to 4 members.

    Attributes:
        id: UUID primary key
        name: Display name for the group
        description: Optional description
        created_by: User ID of group creator
        total_games: Denormalized count of games played
        total_rounds: Denormalized count of rounds played
        last_played_at: When the group last completed a game
    """
...
class GroupMember(Base, UUIDPrimaryKeyMixin):
    ...
    Attributes:
        id: UUID primary key
        group_id: The group this membership belongs to
        user_id: The user who is a member
        role: Role within the group (owner or member)
        joined_at: When the user joined the group
    """
